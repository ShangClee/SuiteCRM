...
Each integration inventory record SHALL include discovery evidence that supports its existence and current API usage.
...
Each integration inventory record SHALL have a status lifecycle with at least: discovered, assessed, planned, migrating, migrated, verified, retired.
